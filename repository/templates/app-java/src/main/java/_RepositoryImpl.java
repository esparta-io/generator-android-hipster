...
public class <%= repositoryName %>RepositoryImpl implements <%= repositoryName %>Repository {
    ...
    public <%= repositoryName %>Repository(<% if (remoteLocal.indexOf('remote') >= 0) { %><%= repositoryName %>RemoteRepository remoteRepository<% } %> <% if ((remoteLocal.indexOf('remote') >= 0) && (remoteLocal.indexOf('local') >= 0)) { %>, <% } %><% if (remoteLocal.indexOf('local') >= 0) { %><%= repositoryName %>LocalRepository localRepository<% } %>) {
